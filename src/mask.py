...
OUTPUTDIRECTORYNAME = 'Path2/Path2-Model Training/Path2 Ribeye Masks'
#OUTPUTDIRECTORYNAMEVAlIDATION = 'Path2/Path2-Model Training/Path2 Fat Thickness Masks'
OUTPUTDIRECTORYNAMEVAlIDATION = 'Path2/Path2-Model Training/Path2 Ribeye Masks'
...
def create_ribeye_mask(image_path):
    ...
    # Convert the single-channel mask to 3 channels for bitwise operation
    finalMask_3channels = cv2.cvtColor(finalMask, cv2.COLOR_GRAY2BGR)

    # Apply the mask to the color image using bitwise_and operation
    #masked_image = cv2.bitwise_and(image, finalMask_3channels)

    # Write the masked image to file
    ribeye_masks_dir = 'Path2/Path2-Model Training/Path2 Ribeye Masks'
